...
namespace SerialPortListener
{
    ...
    public partial class MainForm : Form
    {
        ...
        public void setDataFromClassTableFromDB(DataToUpdate data)
        {
            ...
            //tbMillId.Text = data.millId;
            //tbMillName.Text = data.mill;
            cbbMill.Text = data.mill;
            cbbSite.Text = data.site;
            cbbCarTeam.Text = data.team;
            tbNote.Text = data.note;
            tbWeightOrigin.Text = tonTokg(data.weightOrigin);
            tbQOrigin.Text = numberFormat(data.qOrigin, 2);
            ...
            tbStoneDesc.Text = data.stoneDesc;
        ...
        private void disableBtAfterRead(int mode)
        {
            ...
            else if (mode.Equals(1))
            {
                ...
                //เช็คทะเบียนก่อนถ้าว่างไม่ต้องปิดช่อง
                if (!string.IsNullOrEmpty(tbCarLicense.Text))
                {
                    tbCarLicense.Enabled = false;
                    tbCarCity.Enabled = false;
                }
        ...
        private void autoSave()
        {
            ...
            showErrorEmtyRadioButton(groupBox5);
            //ถ้าเป็นสายยาวไม่ต้องเตือน
        ...
        private void saveAction() {
            ...
            //sql
            OdbcCommand pgCommand = (OdbcCommand)dl.sqlConn().CreateCommand();
            pgCommand.CommandText = "INSERT INTO weight (วันที่, เลขที่เอกสาร, ทะเบียนรถ, จังหวัด, คนขับ, ลูกค้า, น้ำหนักรถ, น้ำหนักรวม, น้ำหนักสินค้า , เลขที่ใบตัก, โรงโม่, ชนิดหิน, จ่ายเงิน, รหัสผู้ชั่ง, รหัสผู้ตัก, ราคาตัน, จำนวณเงิน, ค่าขนส่ง, วันที่ชั่งเข้า, เวลาชั่งเข้า, วันที่ชั่งออก, เวลาชั่งออก, รหัสลูกค้า, ชื่อผู้ชั่ง, ชื่อผู้ตัก, vat, รหัสผู้อนุมัติจ่าย, ชื่อผู้อนุมัติจ่าย, คิว, ชนิดvat, จำนวนเงินสุทธิ, ประเภทหิน, หน้างาน, ทีม, ล้าง, ขนส่ง, หมายเหตุ, carry_type_name, base_weight_station_name, bws, oil_content, site_id, stone_type_id, mill_id, car_team_id, origin_weight, origin_q, line_type, stone_desc)" +
                                     ...
                                     + numberFormat(tbOilContent.Text, 1) + "','" + getComboboxId(cbbSite) + "','" + getComboboxId(cbbStoneType) + "','" + getComboboxId(cbbMill) + "','" + getComboboxId(cbbCarTeam) + "','" + kgToTon(tbWeightOrigin) + "','" + numberFormat(tbQOrigin.Text, 1) + "','" + getLineTypeRadioValue() + "','" + tbStoneDesc.Text + "' )";
            try
            {
                dl.connect();
                OdbcDataReader reader = pgCommand.ExecuteReader();
        ...
        private void saveWeightHistory()
        {
            ...
            //sql
            OdbcCommand pgCommand = (OdbcCommand)dl.sqlConn().CreateCommand();
            pgCommand.CommandText = "INSERT INTO weight_log (weight_id, วันที่, เลขที่เอกสาร, ทะเบียนรถ, จังหวัด, คนขับ, ลูกค้า, น้ำหนักรถ, น้ำหนักรวม, น้ำหนักสินค้า , เลขที่ใบตัก, โรงโม่, ชนิดหิน, จ่ายเงิน, รหัสผู้ชั่ง, รหัสผู้ตัก, ราคาตัน, จำนวณเงิน, ค่าขนส่ง, วันที่ชั่งเข้า, เวลาชั่งเข้า, วันที่ชั่งออก, เวลาชั่งออก, รหัสลูกค้า, ชื่อผู้ชั่ง, ชื่อผู้ตัก, vat, รหัสผู้อนุมัติจ่าย, ชื่อผู้อนุมัติจ่าย, คิว, ชนิดvat, จำนวนเงินสุทธิ, ประเภทหิน, หน้างาน, ทีม, ล้าง, ขนส่ง, หมายเหตุ, carry_type_name, base_weight_station_name, oil_content, site_id, stone_type_id, mill_id, car_team_id, origin_weight, origin_q, line_type, stone_desc)" +
                                     ...
                                     + numberFormat(tbOilContent.Text, 1) + "','" + getComboboxId(cbbSite) + "','" + getComboboxId(cbbStoneType) + "','" + getComboboxId(cbbMill) + "','" + getComboboxId(cbbCarTeam) + "','" + kgToTon(tbWeightOrigin) + "','" + numberFormat(tbQOrigin.Text, 1) + "','" + getLineTypeRadioValue() + "','" + tbDocNum.Text + "' )";
            try
            {
                dl.connect();
                OdbcDataReader reader = pgCommand.ExecuteReader();
        ...
        private void updateAction()
        {
            ...
            pgCommand.CommandText = "UPDATE weight SET ทะเบียนรถ = '" + tbCarLicense.Text.TrimEnd() + "' , จังหวัด = '" + tbCarCity.Text + "' , คนขับ = '" + tbDriverName.Text + "', ลูกค้า = '" + tbCustomerName.Text + "' , น้ำหนักรถ = '" + kgToTon(tbWeightIn) + "' , น้ำหนักรวม = '" + kgToTon(tbWeightOut) + "'" +
                                    ...
                                    "' , origin_weight = '" + kgToTon(tbWeightOrigin) + "' , origin_q = '" + numberFormat(tbQOrigin.Text, 1) + "' , line_type = '" + getLineTypeRadioValue() + "' , stone_desc = '" + tbStoneDesc.Text +
        ...
        private void disableAfterSave()
        {
            ...
            //เปลี่ยนสีปุ่ม btSave หลังจาก save
