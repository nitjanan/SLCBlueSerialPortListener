...
namespace SerialPortListener
{
    ...
    public partial class MainForm : Form
    {
        ...
        private void saveAction() {
            ...
            //sql
            OdbcCommand pgCommand = (OdbcCommand)dl.sqlConn().CreateCommand();
            pgCommand.CommandText = "INSERT INTO weight (วันที่, เลขที่เอกสาร, ทะเบียนรถ, จังหวัด, คนขับ, ลูกค้า, น้ำหนักรถ, น้ำหนักรวม, น้ำหนักสินค้า , เลขที่ใบตัก, โรงโม่, ชนิดหิน, จ่ายเงิน, รหัสผู้ชั่ง, รหัสผู้ตัก, ราคาตัน, จำนวณเงิน, ค่าขนส่ง, วันที่ชั่งเข้า, เวลาชั่งเข้า, วันที่ชั่งออก, เวลาชั่งออก, รหัสลูกค้า, ชื่อผู้ชั่ง, ชื่อผู้ตัก, vat, รหัสผู้อนุมัติจ่าย, ชื่อผู้อนุมัติจ่าย, คิว, ชนิดvat, จำนวนเงินสุทธิ, ประเภทหิน, หน้างาน, ทีม, ล้าง, ขนส่ง, หมายเหตุ, carry_type_name, base_weight_station_name, bws, oil_content, site_id, stone_type_id, mill_id, car_team_id, is_s, do_id, do_doc_no)" +
                                     ...
                                     + numberFormat(tbOilContent.Text, 1) + "','" + getComboboxId(cbbSite) + "','" + getComboboxId(cbbStoneType) + "','" + getComboboxId(cbbMill) + "','" + getComboboxId(cbbCarTeam) + "','" + cbS.Checked + "','" + tbDoId.Text + "','" + tbDoDocNo.Text + "' )";
            try
            {
                dl.connect();
                OdbcDataReader reader = pgCommand.ExecuteReader();
        ...
        private void updateAction()
        {
            ...
            pgCommand.CommandText = "UPDATE weight SET ทะเบียนรถ = '" + tbCarLicense.Text.TrimEnd() + "' , จังหวัด = '" + tbCarCity.Text + "' , คนขับ = '" + tbDriverName.Text + "', ลูกค้า = '" + tbCustomerName.Text + "' , น้ำหนักรถ = '" + kgToTon(tbWeightIn) + "' , น้ำหนักรวม = '" + kgToTon(tbWeightOut) + "'" +
                                    ...
                                    "' , site_id = '" + getComboboxSiteUpdate() + "' , stone_type_id = '" + getComboboxStoneTypeUpdate() + "' , mill_id = '" + getComboboxMillUpdate() + "' , car_team_id = '" + getComboboxCarTeamUpdate() + "' , is_s = '" + cbS.Checked + "' , do_id = '" + tbDoId.Text + "' , do_doc_no = '" + tbDoDocNo.Text +
        ...
        private void updateDeliveryOrder()
        {
            ...
            //sql
            OdbcCommand pgCommand = (OdbcCommand)dl.sqlConn().CreateCommand();
            pgCommand.CommandText = "UPDATE delivery_order SET car_company_tot = '" + calculateDOTotal(tbDoId.Text, 1) + "' , car_customer_tot = '" + calculateDOTotal(tbDoId.Text, 2) +
                                    "' , qty_tot = '" + calculateQtyTotal(tbDoId.Text) +
                                    "' WHERE delivery_date = '" + dtDate.Value.ToString("yyyy-MM-dd") + "' AND do_id = " + tbDoId.Text + " ; ";
        ...
        {
            MessageBox.Show("calculateDOTotal");
            if (mode == 1)
        ...
        {
            MessageBox.Show("calculateQtyTotal");
            ...
            sql.Append("WHERE weight.do_id = '"+ tbDoId.Text + "' ;");
        ...
        private string getDoFromSql(string do_id, string carry_type_name) {
            ...
            MessageBox.Show("getDoFromSql 1");

            string count_id = "";

            //sql find company
            OdbcCommand pgCommand = (OdbcCommand)dl.sqlConn().CreateCommand();
            pgCommand.CommandText = "select count(do_id) as count_id from weight where do_id = '" + tbDoId.Text + "' and carry_type_name = '"+ carry_type_name +"' ";
            try
            {
                dl.connect();
                OdbcDataReader reader = pgCommand.ExecuteReader();
            ...
            MessageBox.Show("getDoFromSql 2");
            return count_id;
        }
