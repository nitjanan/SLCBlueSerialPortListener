...
namespace SerialPortListener
{
    public partial class ucReport : UserControl
    {
        ...
        private void btPrintStoneType_Click(object sender, EventArgs e)
        {
            //sql
            dl.connect();
            string sql = "select * from weight where วันที่ BETWEEN '" + dtFromStoneType.Value.ToString("yyyy-MM-dd") + "' AND '" + dtToStoneType.Value.ToString("yyyy-MM-dd") + "' AND NOT น้ำหนักรวม = 0.00 AND NOT โรงโม่ = ' ' AND NOT รหัสลูกค้า LIKE '10%' AND (NOT site_id = '300PL' OR site_id IS NULL) ";
